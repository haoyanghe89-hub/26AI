...
const DB_PATH = DATABASE_URL.startsWith('sqlite:')
  ? DATABASE_URL.slice('sqlite:'.length)
...
export async function loadUserState(userId) {
  if (USE_POSTGRES) return loadUserStatePostgres(userId)
  ensureDb()
  const sessions = db
    .prepare(
...
export async function saveUserState(userId, state) {
  if (USE_POSTGRES) return saveUserStatePostgres(userId, state)
  ensureDb()
...
async function loadUserStatePostgres(userId) {
  ensurePostgres()
  const sessions = (
    await pgPool.query(
      `SELECT id, title, tags_json, tags_infer_attempts, summary_json, created_at, updated_at
  ...
  return {
    empty: sessions.length === 0 && (await countUserRowsPostgres(userId)) === 0,
    sessions,
    promptTemplates: await loadPromptAssetsPostgres(userId, 'template'),
    customAgents: await loadPromptAssetsPostgres(userId, 'agent'),
    promptWorkflows: await loadPromptAssetsPostgres(userId, 'workflow'),
    pets: await loadStateCollectionPostgres(userId, 'pet'),
    healthLogs: await loadStateCollectionPostgres(userId, 'health_log'),
    careReminders: await loadStateCollectionPostgres(userId, 'care_reminder'),
    carePlans: await loadStateCollectionPostgres(userId, 'care_plan'),
    settings: await loadSettingsPostgres(userId),
    projects: (
      await pgPool.query(
        'SELECT value_json FROM project_metadata WHERE user_id = $1 ORDER BY updated_at DESC',
        [userId],
      )
    ).rows
      .map((row) => parseJson(row.value_json, null))
...
async function saveUserStatePostgres(userId, state) {
  ensurePostgres()
  const now = new Date().toISOString()
  const sessions = Array.isArray(state?.sessions) ? state.sessions : []
  const promptTemplates = Array.isArray(state?.promptTemplates) ? state.promptTemplates : []
  ...
  try {
    ...
    await client.query('DELETE FROM prompt_assets WHERE user_id = $1', [userId])
    await client.query('DELETE FROM user_settings WHERE user_id = $1', [userId])
    await client.query('DELETE FROM project_metadata WHERE user_id = $1', [userId])
    await client.query('DELETE FROM pet_state_collections WHERE user_id = $1', [userId])
    ...
    await savePromptAssetsPostgres(client, userId, 'template', promptTemplates, now)
    await savePromptAssetsPostgres(client, userId, 'agent', customAgents, now)
    await savePromptAssetsPostgres(client, userId, 'workflow', promptWorkflows, now)
    await saveStateCollectionPostgres(client, userId, 'pet', pets, now)
    await saveStateCollectionPostgres(client, userId, 'health_log', healthLogs, now)
    await saveStateCollectionPostgres(client, userId, 'care_reminder', careReminders, now)
    await saveStateCollectionPostgres(client, userId, 'care_plan', carePlans, now)

    for (const [key, value] of Object.entries(settings)) {
      await client.query(
        'INSERT INTO user_settings (user_id, key, value_json, updated_at) VALUES ($1, $2, $3, $4)',
        [userId, key, safeJson(value), now],
      )
    }
    ...
    for (const project of projects) {
      if (!project?.id) continue
      await client.query(
        'INSERT INTO project_metadata (user_id, id, value_json, updated_at) VALUES ($1, $2, $3, $4)',
        [userId, String(project.id), safeJson(project), String(project.updatedAt || now)],
      )
    }
    ...
    console.error('[persistence] saveUserState postgres error:', error)
    throw error
  } finally {
    client.release()
  }
...
async function loadPromptAssetsPostgres(userId, type) {
  return (
    await pgPool.query(
      `SELECT value_json
       FROM prompt_assets
...
async function savePromptAssetsPostgres(client, userId, type, assets, now) {
  for (const asset of assets) {
    if (!asset?.id) continue
    await client.query(
      'INSERT INTO prompt_assets (user_id, type, id, value_json, updated_at) VALUES ($1, $2, $3, $4, $5)',
      [userId, type, String(asset.id), json(asset), String(asset.updatedAt || now)],
    )
  }
}

async function loadStateCollectionPostgres(userId, type) {
  return (
    await pgPool.query(
      `SELECT value_json
...
async function saveStateCollectionPostgres(client, userId, type, items, now) {
  for (const item of items) {
    if (!item?.id) continue
    await client.query(
      'INSERT INTO pet_state_collections (user_id, type, id, value_json, updated_at) VALUES ($1, $2, $3, $4, $5)',
      [userId, type, String(item.id), safeJson(item), String(item.updatedAt || item.createdAt || now)],
    )
  }
}

async function loadSettingsPostgres(userId) {
  return Object.fromEntries(
    (await pgPool.query('SELECT key, value_json FROM user_settings WHERE user_id = $1', [userId])).rows.map(
      (row) => [row.key, parseJson(row.value_json, null)],
    ),
  )
}

async function countUserRowsPostgres(userId) {
  const result = await pgPool.query(
    `SELECT
      (SELECT COUNT(*) FROM prompt_assets WHERE user_id = $1) +
      (SELECT COUNT(*) FROM user_settings WHERE user_id = $1) +
      (SELECT COUNT(*) FROM project_metadata WHERE user_id = $1) +
      (SELECT COUNT(*) FROM pet_state_collections WHERE user_id = $1) AS total`,
    [userId],
  )
...
function ensurePostgres() {
  if (!pgPool) throw new Error('PostgreSQL persistence has not been initialized')
}
