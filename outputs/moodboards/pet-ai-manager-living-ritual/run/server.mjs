...
const codexExecRunner =
  process.env.CREATIVE_PRODUCTION_CODEX_EXEC_RUNNER || runtimeConfig.codexExecRunner || ''
...
async function copyTextToClipboard(text) {
  const args =
    clipboardCommand === 'osascript' ? ['-e', `set the clipboard to ${JSON.stringify(text)}`] : [text]
  await new Promise((resolve, reject) => {
...
async function generateImages(images) {
  ...
  const persistedItems = await persistGeneratedItems(
    images.flatMap((item) => {
...
async function runCodexExecBatch(jobs) {
  ...
  const execResult = await new Promise((resolve) => {
    execFile(
      ...
      [
        ...
        codexWorkspace,
        '--max-concurrency',
        String(effectiveImageMaxConcurrency),
        '--max-attempts',
        imageMaxAttempts,
        '--timeout-seconds',
        imageTimeoutSeconds,
        '--preflight-timeout-seconds',
        imagePreflightTimeoutSeconds,
        '--codex-bin',
        codexBin,
        '--sandbox',
        codexSandbox,
  ...
  if (!(await fileExists(summaryPath))) {
    ...
    const message =
      ...
      'Image generation failed before writing a batch summary.'
...
async function readStreamWithCachedImages() {
  const stream = await readStoredStream()
  stream.items = await Promise.all(
    stream.items.map(async (item) => {
      const filename = cacheName(item)
      const filePath = path.join(generatedDir, filename)
      if (await fileExists(filePath)) {
...
async function serveFile(req, res, pathname) {
  ...
  res.writeHead(200, {
    ...
    'Cache-Control': normalized.startsWith('/generated/')
