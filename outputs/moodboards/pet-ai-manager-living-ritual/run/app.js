...
const EXPLORE_ROUTE_PROMPT_CONTRACTS = {
  ...
  shots:
    '$shot-explorer Explore camera shots, crops, and visual routes for this business visual brief, preserving the provided context and constraints.',
}
    ...
    options: [
      {
        id: 'positioning',
        label: 'Positioning',
        description: 'Clarify audience and occasion.',
        prompt:
      ...
      },
      {
        id: 'mood-boards',
        label: 'Mood boards',
        description: 'Image-first visual territories.',
        prompt:
          '$moodboard-explorer Build an image-first mood board for this business visual brief. Start from the provided context and ask only for missing constraints that block generation.',
      },
      {
        id: 'scenes',
        label: 'Scenes',
        description: 'See the offer in real contexts.',
        prompt:
          '$scene-explorer Explore realistic scene directions for this business visual brief, preserving the provided context and constraints.',
      },
      {
        id: 'offers',
        label: 'Offers',
        description: 'Test the core offer across families.',
        prompt:
          '$offer-explorer Explore structured offer directions for this business visual brief, preserving the provided context and constraints.',
      },
      {
        id: 'ads',
        label: 'Ads',
        description: 'Explore ad directions.',
        prompt: EXPLORE_ROUTE_PROMPT_CONTRACTS.ads,
      },
      {
        id: 'shots',
        label: 'Shots',
        description: 'Try camera angles and crops.',
        prompt: EXPLORE_ROUTE_PROMPT_CONTRACTS.shots,
      },
    ],
  },
]
...
function appendTargetFromPayload(payload) {
  ...
  const source =
    payload.appendTarget && typeof payload.appendTarget === 'object' ? payload.appendTarget : payload
...
function currentBoardStateIdentity() {
  const runDirectory = normalizeStateIdentityPart(
    boardAppendTarget?.runDirectory || window.MOODBOARD_RUN_DIRECTORY || '',
  )
  const streamPath = normalizeStateIdentityPart(boardAppendTarget?.streamPath || '')
  const fallbackKey = normalizeStateIdentityPart(
    window.location.pathname || currentStream.meta?.title || 'moodboard',
  )
...
function savedStateMatchesCurrentBoard(savedState) {
  ...
  const savedStreamPath = normalizeStateIdentityPart(
    savedState.streamPath || savedState.paths?.streamPath || '',
  )
...
function itemIdsFromPayload(payload) {
  ...
  return (stream?.items || [])
    .map((item) => String(item.id || item.title || item.imageUrl || ''))
    .filter(Boolean)
}
...
function boardSignature(payload = {}) {
  ...
  const itemIds =
    Array.isArray(payload.itemIds) && payload.itemIds.length
      ? payload.itemIds.map(String)
      : itemIdsFromPayload(payload)
  const itemCount = Number(
    payload.totalItemCount || payload.itemCount || itemIds.length || streamItems.length || 0,
  )
  return [
...
function applyMcpPayload(payload = mcpWidgetPayload()) {
  ...
  applySavedRunState(payload.restoredState)
  applyMcpWidgetState()
...
async function loadMcpMoodboardPages(payload) {
  ...
  try {
    ...
    if (loadedItems.length > 0) {
      ...
      refreshGenerationSeedItems()
      applySavedRunState(loadedState)
...
function shouldShowSavedMoodboardLoading() {
  return (
    streamItems.length === 0 && Boolean(boardAppendTarget?.runDirectory) && expectedMcpMoodboardItemCount > 0
  )
}
...
function sourceImageDimensions(item) {
  ...
  const renderedImage =
    currentImage?.id === item?.id && viewerImage?.naturalWidth
      ? viewerImage
...
function sourceImageLines(item) {
  ...
  if (item.imageUrl && item.imageUrl !== item.sourceImageUrl)
    lines.push(`  Preview image URL: ${item.imageUrl}`)
...
function annotationEditGuidanceLines(kind, annotationLines) {
  const isAnnotationContext =
    kind === 'creative-production-moodboard-annotation' || annotationLines.length > 0
...
function openViewerAnnotationComposer(item, triggerEvent) {
  ...
  composer.addEventListener('submit', (event) => {
    ...
    void sendMoodboardReferences({
      ...
      successMessage:
        existingAnnotationCount > 0 ? 'Appended annotation to chat.' : 'Added annotation to chat.',
    }).catch((error) => {
...
function applyMcpWidgetState() {
  ...
  if (savedState && typeof savedState === 'object') applySavedRunState(savedState)
}

function applySavedRunState(savedState) {
  ...
  const availableIds = new Set(streamItems.map((item) => item.id))
  if (Array.isArray(savedState.selectedImageIds)) {
...
async function stageHttpAction({
  ...
  const copied =
    Boolean(result.copied) || (await copyContinuationPrompt(result.continuationPrompt).catch(() => false))
...
function normalizeIntakePayload(payload = {}) {
  ...
  return {
    ...
    summary: String(
      payload.summary || 'Pick a few cues. I will use them as the creative brief for the first image set.',
    ),
    context: payload.context && typeof payload.context === 'object' ? payload.context : {},
    mode: String(payload.mode || payload.kind || 'moodboard'),
    groups: normalizeIntakeGroups(
      payload.groups || payload.categories,
      String(payload.mode || payload.kind || 'moodboard'),
    ),
...
function renderSelectionState() {
  ...
  selectionActionToolbar.setAttribute(
    'aria-label',
    count > 0 ? `Actions for ${selectedImageCountLabel(count)}` : 'Selected image actions',
  )
  renderExportAction()
  notifyHostResize()
}
...
function followUpSender() {
  if (
    window.creativeProductionMcp &&
    typeof window.creativeProductionMcp.sendFollowUpMessage === 'function'
  ) {
...
function conciseImageFocus(item) {
  ...
  return String(value).replace(/\s+/g, ' ').replace(/[.]+$/g, '').trim().slice(0, 120) || 'the source image'
}
...
function remixFocusLabel(item) {
  ...
  const meaningful = words.filter(
    (word) => !['and', 'the', 'with', 'from', 'into', 'for', 'image'].includes(word.toLowerCase()),
  )
  return (meaningful.slice(0, 3).join(' ') || 'Selected Image').replace(/\b\w/g, (letter) =>
    letter.toUpperCase(),
  )
}

const remixNounStopWords = new Set([
  'cue',
  'detail',
  'echo',
  'flash',
...
function normalizeRemixOption(option, index, slot) {
  ...
  const description = String(
    option.description || option.summary || option.detail || slot.promptHint || '',
  ).trim()
  return {
...
function imageProvidedRemixOptions(item, slot) {
  const source =
    item?.remixSuggestions ||
    item?.remix_suggestions ||
    item?.variationSuggestions ||
    item?.variation_suggestions
  if (!source) return []
...
function contextualRemixLabel(slot, option, focusNoun) {
  ...
  if (slot.id === 'style') {
    const styleLead =
      option.id === 'documentary-realism'
        ? 'Documentary'
        : option.id === 'dramatic-contrast'
          ? 'Dramatic'
          : 'Editorial'
...
function contextualRemixDescription(slot, option, focus) {
  ...
  if (slot.id === 'character')
    return `${option.description} Preserve the mood and composition around ${focus}.`
...
function remixOptionSeed(slot, item) {
  const source = [slot.id, item?.title, item?.caption, item?.motif, item?.tone, item?.prompt]
    .filter(Boolean)
    .join('|')
...
function dynamicRemixSuggestions(item) {
  return Object.fromEntries(remixSlots.map((slot) => [slot.id, dynamicRemixOptionsForSlot(slot, item)]))
}
...
async function fileForMoodboardItem(item) {
  const blob = await pngBlobForImage(item)
  const safeId =
    String(item?.id || 'moodboard-image')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, '') || 'moodboard-image'
...
function moodboardContextText(items, kind = null, extra = {}) {
  ...
  const itemLines = items.map((item, index) => {
    const title = item.title || item.id || `Image ${index + 1}`
    const caption = item.caption ? ` - ${item.caption}` : ''
    return [`${index + 1}. ${title}${caption} (id: ${item.id})`, ...sourceImageLines(item)].join('\n')
  })
  ...
  if (explicitAnnotations.length) {
    explicitAnnotations.forEach((annotation, index) => {
      ...
      if (annotation.point || annotation.annotationPoint) {
        ...
        annotationLines.push(
          `  Spot: x=${annotationPoint.x}, y=${annotationPoint.y} in normalized image coordinates from the top-left.`,
        )
      }
    })
    ...
    if (extra.annotationPoint) {
      annotationLines.push(
        `- Current annotated spot: x=${extra.annotationPoint.x}, y=${extra.annotationPoint.y} in normalized image coordinates from the top-left.`,
      )
    }
  }
  includedGroups.forEach((group, index) => {
    ...
    if (group.point) {
      annotationLines.push(
        `  Spot: x=${group.point.x}, y=${group.point.y} in normalized image coordinates from the top-left.`,
      )
    }
  })
...
async function sendMoodboardReferences({
  ...
  if (mcpUpdateModelContext) {
    ...
    const textBlock = {
      type: 'text',
      text: moodboardContextText(targetItems, structuredKind, structuredContent),
    }
...
async function sendMoodboardFollowUp({ action, items, prompt, successMessage }) {
  const targetItems = items.filter(Boolean)
  ...
  if (mcpSendFollowUpMessage) {
    await mcpSendFollowUpMessage({
      ...
      content: [{ type: 'text', text: resolvedPrompt }, ...imageBlocks],
    })
    feedStatus.textContent = successMessage
    showToast(successMessage)
    scheduleRunStateSave({ interaction: action })
...
function moodboardStructuredContent(items, kind = null, extra = {}) {
  ...
  return {
    kind:
      kind ||
      (includedAnnotationGroups.length > 0
        ? 'creative-production-moodboard-annotated-context'
        : 'creative-production-moodboard-selection'),
    attachmentCount: items.length,
...
function appendItems(items) {
  items.forEach((item) => {
    ...
    select.setAttribute(
      'aria-label',
      `${selectedImageIds.has(item.id) ? 'Deselect' : 'Select'} ${item.title || 'image'}`,
    )
    select.setAttribute(
      'title',
      `${selectedImageIds.has(item.id) ? 'Deselect' : 'Select'} ${item.title || 'image'}`,
...
function shouldGenerateMoreImages() {
  return canGenerateMoreImages && !isGeneratingMoreImages && generationSeedItems.length > 0
}
...
async function animateViewerTransition(sourceImage, targetImage, src, update) {
  ...
  try {
    ...
    const animation = transitionImage.animate([transitionFrame(fromState), transitionFrame(toState)], {
      duration: 360,
...
async function closeViewer() {
  ...
  await animateViewerTransition(
    viewerImage,
    imageForTrigger(focusTarget),
    currentImage?.src || viewerImage.currentSrc || viewerImage.src,
    async () => {
      setViewerOpen(false)
    },
  )
...
async function init() {
  const stream =
    streamFromMcpPayload(mcpWidgetPayload()) ||
    EMBEDDED_STREAM ||
    (await loadJson('/api/stream', null)) ||
    (await loadJson('data/stream-static.json', null)) ||
    (await loadJson('data/stream.json', FALLBACK_STREAM))
  currentStream = stream
